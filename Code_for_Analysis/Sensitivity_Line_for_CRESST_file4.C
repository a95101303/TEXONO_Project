...
void Sensitivity_Line_for_CRESST_file4()//CDEX:Threshold=160eV, TEXONO:Threshold=200eV
{
    ...
    int Bent_Type=1; string Bent_or_Not_Type[2]={"_Comparison",""};
...
for(int kkk=1;kkk<2;kkk++)
    {
        ...
        double Mass=0;
    for(int FILE=20; FILE<22; FILE++){//Open1
        ...
        {
            char fout_name[300];
            sprintf(fout_name,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/%s_STS_Bent%s_Earth.root",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),Bent_or_Not_Type[Bent_Type].c_str()));
            TFile *fout=new TFile(fout_name,"recreate");
            Threshold_Plot_2->Write();
            c4->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/%s_STS_Bent%s_Earth.pdf",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),Bent_or_Not_Type[Bent_Type].c_str()));
        }
    }
}
