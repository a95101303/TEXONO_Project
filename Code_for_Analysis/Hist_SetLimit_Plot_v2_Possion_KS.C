        ...
        for(int lll=3; lll<4; lll++){
            ...
            */
            ...
            for(int kkk=0 ; kkk<Simulated_Event_Number ; kkk++)
            {
                ...
                /*
                cout << "Velocity_X: " << Velocity_X[kkk] << endl;
                cout << "Velocity_Y: " << Velocity_Y[kkk] << endl;
        ...
        double Path_Length_For_Three_Components[4]={Bool_If_Earth_Check,Cement_Length,Reactor_Length_Total,Reactor_Length_Water};//1 for Air check, 2 for the Cement, 3 for the Reactor
            ...
            for(int kkk=0; kkk<Simulated_Event_Number; kkk++)
            {
                ...
                else
                {
                    ...
                    int Collision_Expectation_EARTH_Int = Collision_Expectation_EARTH[kkk];
            ...
            char fout_name[100];
            sprintf(fout_name,Form("Sigma_SI_Flux_KS_10GeV/%ie_M%i.root",ppp,27+lll));
            ...
            //double PX=0; double PY=1; double PZ=sqrt(1-PX*PX-PY*PY);
            //Collision_Time_ATM(1,1,1,1);
            /*
            double PY=0; double PZ=1; double PX=sqrt(1-PY*PY-PZ*PZ);

            
            for(int kkk=0; kkk<11; kkk++)
            {
                cout << "kkk: " << kkk << endl;
               // double PZ=0; double PY=(1-0.01*kkk);double PX=sqrt(1-PY*PY-PZ*PZ);
                //double PY=(0.01*kkk); double PZ=(0.9+0.01*kkk); double PX=sqrt(1-PY*PY-PZ*PZ);
                double PY=(1-0.1*kkk); double PZ=(0+0.1*kkk); double PX=sqrt(1-PY*PY-PZ*PZ);
                double Bool_If_Earth_Check= Bool_If_Earth( PX, PY, PZ);
                double Cement_Length = KS_Cement_Path_Length( PX, PY, PZ);
                double Reactor_Length = KS_Reactor_Path_Length( PX, PY, PZ);
        double Path_Length_For_Three_Components[3]={Bool_If_Earth_Check,Cement_Length,Reactor_Length};//1 for Air check, 2 for the Cement, 3 for the Reactor
                double *A=KS_Collision_Time_EARTH(1,PY,PZ,1,1,Path_Length_For_Three_Components);
                cout << "KS_Collision_Time_ATM(): " << KS_Collision_Time_ATM(1,PY,PZ,1,1,Path_Length_For_Three_Components,A[1]) << endl;
            }
            */
        }
    }
