...
void Hist_SetLimit_Plot_v2_Neutrino()
{
...
const int Number_mx_Candidate = 10;
double A = AXe;
...
double WIMP_mx[Number_mx_Candidate]={10,15,20};
double WIMP_max_T[Number_mx_Candidate];
for(int jjj=0; jjj<Number_mx_Candidate ; jjj++)
{
    WIMP_max_T[jjj] = 1000.0*max_recoil_A(WIMP_mx[jjj], 779.135*1000.0/2.99792458e8, AXe)+10; //keV
}
...
double RE_Nu_DATA_Err[Neutrino_Data_element]; double RE_Nu_Rate_Err[Neutrino_Data_element]; double RE_Nu_Bin_Number[Neutrino_Data_element];
        ...
        for(int jjj=0 ; jjj<Number_mx_Candidate ; jjj++)
        {
            for(int i=0;i<reso_T;i++)
            {
                ...
                for(int j=0;j<2000;j++)
                {
                    ...
                    {
                        recoilX[jjj][i] = recoilX[jjj][i] + (1/QF_Xe(T[jjj][i]))*fdsigma_dT_keV(WIMP_mx[jjj], 1e-40, v, AXe, T[jjj][i])
                        *N_atom_Xe_1kg*(rohx/WIMP_mx[jjj])*v_cm_day*(1/(sum))*velo_dist_Ave[j][3];
                    }
                }
            }
...
cdexdata->SetMarkerSize(1);
    
TGraph *cdexdata1 = new TGraph(Neutrino_Data_element,RE_Nu_DATA,RE_Nu_Rate);
cdexdata1->SetName("gcpkkdX1");
cdexdata1->SetMarkerColor(4);
cdexdata1->SetMarkerStyle(21);
cdexdata1->SetMarkerSize(0.1);
    cdexdata1->SetLineColor(3);
    cdexdata1->GetXaxis()->SetLimits(0,15);
...
leg->AddEntry(gcpkkdX1,Form("m_{#chi}=10GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[0],SigmaSI_Scale[0]),"lp");
leg->AddEntry(gcpkkdX2,Form("m_{#chi}=15GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[1],SigmaSI_Scale[1]),"lp");
...
leg->Draw();
    ...
    //
c1->SetLogy();
c1->Print("Neutrino_Cases/Plot_ZGe.png");
//68% (1sigma) => 90% (2sigma)
}
