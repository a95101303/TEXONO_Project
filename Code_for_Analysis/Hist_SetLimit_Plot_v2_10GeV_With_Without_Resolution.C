...
void Hist_SetLimit_Plot_v2_10GeV_With_Without_Resolution()
{
...
double recoil_Data[Number_mx_Candidate][Data_element];
double v;
double WIMP_mx[Number_mx_Candidate]={2.34,5,0.1785,50,100}; //2.34
double WIMP_max_T[Number_mx_Candidate];
for(int jjj=0; jjj<Number_mx_Candidate ; jjj++)
{
    WIMP_max_T[jjj] = 1000.0*max_recoil_A(WIMP_mx[jjj], 779.135000*1000.0/2.99792458e8, A)+1.8; //keV
}
    
for(int jjj=0 ; jjj<100 ; jjj++)
{
    //cout << "Mass: " << 1.9+0.01*jjj << endl;
    //cout << "WIMP_max_T[jjj]: " << max_recoil_A_keV(1.9+0.01*jjj, 779.135000*1000.0/2.99792458e8, A) << endl;
    //cout << "TQF(WIMP_max_T[jjj]): " << TQF(max_recoil_A_keV(1.9+0.01*jjj, 779.135000*1000.0/2.99792458e8, A)) << endl;
}
//double mx = 50.0; // GeV
//========For Data
...
double ConstantX[6];
    ...
    //===============================================================
        ...
        for(int jjj=0 ; jjj<Number_mx_Candidate ; jjj++)
        {
            for(int i=0;i<reso_T;i++)//For theory
            {
                ...
                for(int j=0;j<2000;j++)
                {
                    ...
                    {
recoilX[jjj][i] = recoilX[jjj][i] + rate_scale_QF(T[jjj][i])*fdsigma_dT_keV(WIMP_mx[jjj], 1e-40, v, A, T[jjj][i])*N_atom_Ge_1kg*(rohx/WIMP_mx[jjj])*v_cm_day*(1/(sum))*velo_dist_Ave[j][3];
    ...
    double Factor_Recoil[Number_mx_Candidate][1000];
    ...
    for(int kkk=0 ; kkk<Number_mx_Candidate ; kkk++)
    {
        ...
        for(int jjj=0 ; jjj<dm_spec_resolution ; jjj++)
        {
            ...
            if(kkk==2 and T_QF[2][jjj]>0.20)
            {
                cout << "T_QF:"             <<    T_QF[2][jjj] << endl;
                cout << "Factor1[2][jjj]: " << Factor1[2][jjj] << endl;
            }
        }
    ...
    double Initial_Factor1[Number_mx_Candidate];
    ...
    for(int kkk=0 ; kkk<Number_mx_Candidate ; kkk++)
    {
        Initial_Factor1[kkk]=0;
        for(int iii=0 ; iii<Data_element ; iii++)
        {
            double Ratio1 = Factor1[kkk][iii]*(1/Initial_Factor[kkk])/(recoilX[kkk][iii]);
            if(abs(Ratio1)>Initial_Factor1[kkk] && abs(1/Ratio1)!=0)
            {
                Initial_Factor1[kkk] = Ratio1;
            }
        }
    }
    for(int kkk=0 ; kkk<Number_mx_Candidate ; kkk++)
    {
        ...
        cout << "1/Ratio1[kkk]: " << Initial_Factor1[kkk] << endl;
    ...
    for(int kkk=0 ; kkk<Number_mx_Candidate ; kkk++)
    {
        ...
        for(int jjj=0 ; jjj<1000 ; jjj++)
        {
            if(kkk==2)
            {
                cout << "Factor1: " << Factor1[kkk][jjj] << "RecoilX: " <<recoilX[kkk][jjj] << endl;
                cout << "T[jjj][i]: " << T[kkk][jjj] << "T_QF[jjj][i]: " << T_QF[kkk][jjj] << endl;
            }
                Factor2[kkk][jjj] = Factor1[kkk][jjj]*1/Initial_Factor[kkk];
            Factor_Recoil[kkk][jjj] = (recoilX[kkk][jjj])*Initial_Factor1[kkk];
            //cout << "1/Initial_Factor[kkk]: " << 1/Initial_Factor[kkk] << endl;
            //cout << "Factor1[kkk][jjj]*(1/Initial_Factor[kkk])/(recoilX[kkk][jjj]): " << Factor1[kkk][jjj]*(1/Initial_Factor[kkk])/(recoilX[kkk][jjj]) << endl;
        }
    }
...
cdexdata->GetXaxis()->SetRangeUser(0,1);
...
TGraph *gcpkkdX1 = new TGraph(reso_T,T_QF[0],Factor_Recoil[0]);
//gcpkkdX1->SetName("gcpkkdX1");
gcpkkdX1->SetLineColor(3);
TGraph *gcpkkdX2 = new TGraph(reso_T,T_QF[2],Factor2[2]);
...
leg->AddEntry(gcpkkdX1,"2.34GeV(Without Resolution)","");
leg->AddEntry(gcpkkdX2,"0.1785GeV(With Resolution)","lp");
...
leg->Draw();
    ...
    //
c1->Print("10GeV_with_without_resolution.png");
//68% (1sigma) => 90% (2sigma)
}
