...
void Overlap_Plot_SURF_Ge()
{
    ...
    Sigma_SI_Array[0]=2*(1e-28);
    Sigma_SI_With_Threshold_M1[0]=1e-43;
    Sigma_SI_With_Threshold_M3[0]=1e-43;

    int Point_Number=1;
for(int kkk=29;kkk<41;kkk++)//for(int kkk=27;kkk<41;kkk++)
{
    for(int lll=1; lll<10; lll++)//for(int lll=1; lll<10; lll++)
    {
    ...
    {
        cout << "111: " << endl;
        YYY=29;Multiply_Number=(10-lll);
        cout << "Multiply_Number_Int: " << Multiply_Number << endl;
        cout << "YYY: " << YYY << endl;
        sprintf(fname,Form("Sigma_SI_Flux_SURF_2P34GeV/%iP0e_M%i.root",10-lll,YYY));
        Sfname = Form("1P%ie_M%i",10-lll,YYY);
        Addition_Option=0;
    }
    else if(kkk>29 and lll==1)
    {
        cout << "000: " << endl;
        YYY=kkk;Multiply_Number=lll;
        sprintf(fname,Form("Sigma_SI_Flux_SURF_2P34GeV/%iP0e_M%i.root",lll,YYY));
    ...
    double *T_QF_Original_Bef=RecoilX_Event(0,Flux_HIST_Random,Mass,Sigma_SI_Array[Point_Number],0);
    for(int i=0;i<reso_T;i++){
        if(i==0 or i==reso_T-1)cout << "T_QF_Original_Bef_Array[i]: " << T_QF_Original_Bef_Array[i] << endl;
        T_QF_Original_Bef_Array[i]=T_QF_Original_Bef[i];}
            
    double *T_QF_Original_Aft=RecoilX_Event(0,Flux_HIST_Aft_Collision_EARTH,Mass,Sigma_SI_Array[Point_Number],0);
    for(int i=0;i<reso_T;i++){
        if(i==0 or i==reso_T-1)cout << "T_QF_Original_Aft_Array[i]: " << T_QF_Original_Aft_Array[i] << endl;
        T_QF_Original_Aft_Array[i]=T_QF_Original_Aft[i];}
        
        cout << "YES1: " << endl;
    double *Factor1_Original_Bef=RecoilX_Event(1,Flux_HIST_Random,Mass,Sigma_SI_Array[Point_Number],0);
    for(int i=0;i<reso_T;i++){
        if(i==0 or i==reso_T-1)cout << "Factor1_Original_Bef[i]: " << Factor1_Original_Bef[i] << endl;
        Factor1_Original_Bef_Array[i]=(Factor1_Original_Bef[i]);}
        
    double *Factor1_Original_Aft=RecoilX_Event(1,Flux_HIST_Aft_Collision_EARTH,Mass,Sigma_SI_Array[Point_Number],0);
    for(int i=0;i<reso_T;i++){
        if(i==0 or i==reso_T-1)cout << "Factor1_Original_Aft[i]: " << Factor1_Original_Aft[i] << endl;
    ...
    for(int i=0;i<reso_T;i++)
    {
        ...
        if(T_QF_Original_Bef[i]>0.2 and T_QF_Original_Bef[i]<0.3) RecoilX_Event_Original_M3 = RecoilX_Event_Original_M3 + Factor1_Original_Bef_Array[i];
        if(T_QF_Original_Aft[i]>0.2 and T_QF_Original_Aft[i]<0.3) RecoilX_Event_Aft_EARTH_M3 = RecoilX_Event_Aft_EARTH_M3 + Factor1_Original_Aft_Array[i];
    }
   ...
   Sigma_SI_With_Threshold_M3_Error[Point_Number] =Sigma_SI_Array[Point_Number]*Binomial_Error(Event_Number, (RecoilX_Event_Aft_EARTH_M3/RecoilX_Event_Original_M3) );
    ...
    ER_Spectrum_Bef->GetXaxis()->SetRangeUser(0,3);
    ER_Spectrum_Bef->GetYaxis()->SetRangeUser(0,1e+15);
    ...
    ER_Spectrum_Aft->GetXaxis()->SetRangeUser(0,5);
    ER_Spectrum_Aft->GetYaxis()->SetRangeUser(0,1e+15);
 ...
 */
    ...
    leg->AddEntry("",Form("Ratio of >200eV: %.5f",RecoilX_Event_Aft_EARTH_M1/RecoilX_Event_Original_M1),"l");
    leg->AddEntry("",Form("Ratio of >200eV and <250eV: %.5f",RecoilX_Event_Aft_EARTH_M3/RecoilX_Event_Original_M3),"l");
    ...
    leg->Draw();
    c3->SetLogy();
    c3->Print(Form("Recoil_Spectrum/%s_Aft.pdf",Sfname.c_str()));
...
*/
   ...
   // leg->AddEntry(Threshold_Plot_5,"Through Average Density","lP");
    ...
    cout << "Yes?!3" << endl;
    cout << "Point_Number: " << Point_Number << endl;
    c4->Print("P34GeV_EXCLUSION_Plot_1_SURF.pdf");
    
    char fout_name[100];
    sprintf(fout_name,"2P34GeV_EXCLUSION_Plot_1_SURF.root");
