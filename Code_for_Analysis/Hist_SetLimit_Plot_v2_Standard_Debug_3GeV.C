...
void Hist_SetLimit_Plot_v2_Standard_Debug_3GeV()
{
...
double v;
double WIMP_mx[Number_mx_Candidate]={3,3,3,3,3};
...
// Data_implemented_rebi n_2
    ...
    double Data_RE_Rate_Err[4]= {3*1.64458/0.994,3.5*1.64458/0.994,1.5*1.64458/0.994,2*1.64458/0.994};
...
for(int jjj=0 ; jjj<4 ; jjj++)
{
 if(jjj<4){
        ...
        RE_DATA_Err[jjj]= 0;
        RE_Rate_Err[jjj] = Data_RE_Rate_Err[jjj];}
    else{
        RE_DATA[jjj]= 0;
        RE_Rate[jjj]= 0;
        RE_DATA_Err[jjj]= 0;
...
double ConstantX[6];
    ...
    double Factor3[Number_mx_Candidate][1000];
    ...
    for(int kkk=0 ; kkk<Number_mx_Candidate ; kkk++)
    {
        ...
        for(int jjj=0 ; jjj<1000 ; jjj++)
        {
            ...
            Factor3[kkk][jjj] = Factor1[kkk][jjj]*29.5933;
            Factor2[kkk][jjj] = Factor1[kkk][jjj]*1/Initial_Factor[kkk];
            if(kkk==3)
            {
...
cdexdata->GetXaxis()->SetRangeUser(0,0.5);
...
TGraph *gcpkkdX4 = new TGraph(reso_T,T_QF[3],Factor3[2]);
...
leg->AddEntry(gcpkkdX3,Form("m_{#chi}=3GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[2],SigmaSI_Scale[2]),"lp");
leg->AddEntry(gcpkkdX4,Form("m_{#chi}=3GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",2.95933,39),"lp");
//leg->AddEntry(gcpkkdX5,Form("m_{#chi}=4GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[4],SigmaSI_Scale[4]),"lp");

cdexdata->Draw("apE");
CDEX_DATA->Draw("psame");
