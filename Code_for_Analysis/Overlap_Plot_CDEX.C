...
for(int kkk=29;kkk<41;kkk++)//for(int kkk=27;kkk<41;kkk++)
{
    ...
    {
        cout << "111: " << endl;
        YYY=31;Multiply_Number=(10-lll);
        cout << "Multiply_Number_Int: " << Multiply_Number << endl;
        cout << "YYY: " << YYY << endl;
        sprintf(fname,Form("Sigma_SI_Flux_CDEX_2P07GeV/%iP%ie_M%i.root",10-lll,YYY));
        Sfname = Form("P%ie_M%i",10-lll,YYY);
        Addition_Option=0;
    }
    else if(kkk>30 and lll==1)
    {
        cout << "000: " << endl;
    ...
    for(int i=0;i<reso_T;i++)
    {
        ...
        if(T_QF_Original_Bef[i]>0.16 and T_QF_Original_Bef[i]<0.26) RecoilX_Event_Original_M3 = RecoilX_Event_Original_M3 + Factor1_Original_Bef_Array[i];
        if(T_QF_Original_Aft[i]>0.16 and T_QF_Original_Aft[i]<0.26) RecoilX_Event_Aft_EARTH_M3 = RecoilX_Event_Aft_EARTH_M3 + Factor1_Original_Aft_Array[i];
    }
...
*/
   ...
   // leg->AddEntry(Threshold_Plot_5,"Through Average Density","lP");
    ...
    char fout_name[100];
    sprintf(fout_name,"2P07GeV_EXCLUSION_Plot_1_CDEX.root");
