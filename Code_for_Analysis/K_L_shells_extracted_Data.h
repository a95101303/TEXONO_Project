...
float Extract_K_L_shell(float Target_X, vector<float> Constant, vector<float> Mean, vector<float> Error)
{
    ...
    {
        if(Target_X>0.9 and Target_X<1.4){
            cout << "Constant[kkk]*TMath::Exp(-((Target_X-Mean[kkk])*(Target_X-Mean[kkk]))/(2*Error[kkk]*Error[kkk])): " << Constant[kkk]*TMath::Exp(-((Target_X-Mean[kkk])*(Target_X-Mean[kkk]))/(2*Error[kkk]*Error[kkk])) << endl;}
...
vector<vector<float>> K_L_shell_extracted_Data()
{
...
double RE_DATA_Aft[Data_element]; double RE_Rate_Aft[Data_element]; double RE_DATA_Err_Aft[Data_element]; double RE_Rate_Err_Aft[Data_element];
...
//cout << "Gaus6->GetParameter(0)" << Constant0->GetParameter(0) << endl;
...
cdexdata1->GetXaxis()->SetRangeUser(0,13.025);
cdexdata1->GetYaxis()->SetLimits(-20,800);
...
TF1 *Energy_transformed = new TF1("f1","[0]*x",4,11);
Peak_Position->SetName("Peak_Position");
Peak_Position->SetLineColor(1);
Peak_Position->GetXaxis()->SetRangeUser(4,12);
Peak_Position->GetYaxis()->SetLimits(4,12);
...
leg->Draw();
    ...
    //
c1->Print("TEXONO_PEAK_Extract.png");
    
vector<vector<float>> vec;
vec.push_back(RE_DATA_Aft);
vec.push_back(RE_Rate_Aft);
vec.push_back(RE_DATA_Err_Aft);
vec.push_back(RE_Rate_Err_Aft);
