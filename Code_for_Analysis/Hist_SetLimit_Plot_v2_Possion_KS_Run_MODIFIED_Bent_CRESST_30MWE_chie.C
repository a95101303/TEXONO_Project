...
void Hist_SetLimit_Plot_v2_Possion_KS_Run_MODIFIED_Bent_CRESST_30MWE_chie(int Bent_or_not, int Index_Mass, int Simulated_Event_Number, int Index_Sigma, double Sigma_SI)
{
   ...
   // double Velocity_X[Simulated_Event_Number];double Velocity_Y[Simulated_Event_Number];
    ...
    double V_Int_A, V_End_A, V_End_E, V_End_S;
    double V_X, V_Y, V_Z;
    ...
    t1->Branch("V_End_S",&V_End_S,"V_End_S/D");
...
double *ATM_Value =         KS_Collision_Time_ATM_Aft_velocity_with_angle(Bent_or_not_to_be_Bent,kkk,Index_Sigma,Sigma_SI,0,1,Random_Velocity,DM_mx,Path_Length_For_Three_Components,A[0],Direction_of_Velocity);
        ...
        double ID[3]={ATM_Value[7],ATM_Value[8],abs(ATM_Value[9])};//Intermediate_Direction
        ...
        if(Arrival_air==1)
        {
            ...
            {
                V_End_S = KS_30MWE[0];
                Flux_HIST_Aft_Collision_EARTH->Fill(KS_30MWE[0]);
            }
            ...
            if(int(Arrival_30MWE)==1)
            {//Arrival_air_Open
            double *Earth_Value = KS_Collision_Time_Earth_Aft_velocity_with_angle(Bent_or_not_to_be_Bent,kkk,Index_Sigma,Sigma_SI,KS_30MWE[0],DM_mx,ID,IP);
    ...
    char fout_name[100];
    if(Bent_or_not_to_be_Bent==1)sprintf(fout_name,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/%i_STS_Bent_Earth.root",Mass_Point[Index_Mass].c_str(),Index_Sigma));
    if(Bent_or_not_to_be_Bent==0)sprintf(fout_name,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/%i_STS_Bent_Comparison.root",Mass_Point[Index_Mass].c_str(),Index_Sigma));
