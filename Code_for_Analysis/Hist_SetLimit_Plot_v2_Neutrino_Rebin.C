...
double RE_Nu_Rate_bin_Content[Neutrino_Data_element];
  ...
  //  cout << "Real_yp: " << yp << endl;
    for(int jjj=0 ; jjj<2000 ; jjj++)
    {
        if( (xp>=0.0000+jjj*0.05) and (xp<0.0500+jjj*0.05) )
        {
            ...
            if(0.025+jjj*0.05>=0.225)RE_Nu_Rate[jjj] = RE_Nu_Rate[jjj] + yp;
            RE_Nu_Rate_bin_Content[jjj] = RE_Nu_Rate_bin_Content[jjj]+1;
        }
    }
}
...
{
    if(RE_Nu_Rate_bin_Content[jjj]!=0){RE_Nu_Rate[jjj] = RE_Nu_Rate[jjj]/RE_Nu_Rate_bin_Content[jjj];}
    else{RE_Nu_Rate[jjj]=0;}
    //cout << "RE_Nu_Rate_bin_Content[jjj]: " << RE_Nu_Rate_bin_Content[jjj] << endl;
    //cout << "RE_Nu_DATA[jjj]: " << RE_Nu_DATA[jjj] << endl;
...
for(int jjj=0 ; jjj<Neutrino_Data_element ; jjj++)
{
    ...
    if(RE_Nu_DATA[jjj]>=0.225){
        cout << "Yes: " << endl;
        RE_Nu_Rate_Err[jjj] = (sqrt(RE_Nu_Rate[jjj]*365.0*(50.0/1000.0)));}
    else{
        cout << "No: " << endl;
        RE_Nu_Rate_Err[jjj]=0;
       }
    RE_Nu_Rate_Err[jjj] = RE_Nu_Rate_Err[jjj]*(1/(18.25));
    cout << "RE_Nu_Rate_Err[jjj]_Aft: " << RE_Nu_Rate_Err[jjj] << endl;
...
cdexdata->GetXaxis()->SetRangeUser(0,14);
...
TGraphErrors *cdexdata1 = new TGraphErrors(2000,RE_Nu_DATA,RE_Nu_Rate,RE_Nu_DATA_Err,RE_Nu_Rate_Err);
cdexdata1->SetName("cdexdata1");
cdexdata1->SetMarkerColor(4);
cdexdata1->SetMarkerStyle(21);
cdexdata1->GetXaxis()->SetRangeUser(0,3);
cdexdata1->GetYaxis()->SetLimits(-1,3);
...
leg->AddEntry(cdexdata1,Form("m_{#chi}=3GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[0],SigmaSI_Scale[0]),"lp");
leg->AddEntry(gcpkkdX2,Form("m_{#chi}=10GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[1],SigmaSI_Scale[1]),"lp");
leg->AddEntry(gcpkkdX3,Form("m_{#chi}=20GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[2],SigmaSI_Scale[2]),"lp");
leg->AddEntry(gcpkkdX4,Form("m_{#chi}=50GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[3],SigmaSI_Scale[3]),"lp");
leg->AddEntry(gcpkkdX5,Form("m_{#chi}=100GeV, #sigma^{SI}_{#chi N}=%f #times 10^{-%i}",SigmaSI_Scale_Number[4],SigmaSI_Scale[4]),"lp");
...
leg->Draw();
    ...
    //
//c1->SetLogy();
c1->Print("Neutrino_Cases/Plot_Neutrino.png");
//68% (1sigma) => 90% (2sigma)
}
