...
static double DCS_H_array_slope[11];//TMath::Log10()
static double DCS_H_array_b[11];//TMath::Log10()
...
double DCS_H_c1_1GeV_300kms(double ER)//ER(eV)
{
    ...
    for(int KKK=0; KKK<11; KKK++)
    {
        if(ER>DCS_H_array[KKK][0] and ER<DCS_H_array[KKK+1][0])
        {
            Slope_Used=DCS_H_array_slope[KKK];
...
double DCS_H_c1_0P05GeV_300kms(double ER)//ER(eV)
{
    
    const double DCS_H_array[4][2]=//ER(eV),TMath::Log10(DCS)
    {
        14.,-14.,
        19.,-15.,
        23.5,-16.,
...
//Test the fitting line
{
    ...
    */
    ...
    double Length_paper    = 2e5;//2km
    double Max_V          = 784.;
    double Cross_section  = 3e-35;
    double Mx             = 1.;

    double dE_dX_without_velocity = dE_dX_from_others(2,Cross_section,Mx,Max_V,Length_paper,0);
    cout << "Total_Cross_Section: " << dE_dX_without_velocity << endl;
    
    double *Ee_array        = dsigma_dT_from_others(0,Cross_section,Mx,Max_V,Length_paper,0);
    double *dsigma_dT_array = dsigma_dT_from_others(1,Cross_section,Mx,Max_V,Length_paper,0);
    
    double Total=0;
    for(int Ee_array_Number=0; Ee_array_Number<500; Ee_array_Number++)
    {
        cout << "Ee_array: "        << Ee_array[Ee_array_Number] << endl;
        cout << "dsigma_dT_array: " << dsigma_dT_array[Ee_array_Number] << endl;
        Total = Total + 0.1*dsigma_dT_array[Ee_array_Number]*Ee_array[Ee_array_Number];
    }
    ...
    for(int kkk=1; kkk<2; kkk++)
    {
       ...
       // for(int Applied_Hist=0; Applied_Hist<velocity_N; Applied_Hist++)
         for(int Applied_Hist=6; Applied_Hist<7;Applied_Hist++)
        //for(int Applied_Hist=5; Applied_Hist<6; Applied_Hist++)
        //for(int Applied_Hist=3; Applied_Hist<velocity_N; Applied_Hist++)
        {
            ...
            //Draw the differential cross sections(Real line)
            /*
            const int Bin_Number_draw=10000;
            double Ionization_Energy[Bin_Number_draw];
            double DCS_Xe_array[Bin_Number_draw];double DCS_Ge_array[Bin_Number_draw];double DCS_HH_array[Bin_Number_draw];
            //cout << "TMath::Power(10, Linear_Line(12.,A_Slope_HH,B_HH)): " << TMath::Power(10, Linear_Line(12.,A_Slope_HH,B_HH)) << endl;
            double Bin = (480.-12.)/Bin_Number_draw;
            
            for(int KKK=0; KKK<Bin_Number_draw; KKK++)
            {
                double Ionization_Energy_individual= 12.+Bin*(KKK+1.);//eV
                Ionization_Energy[KKK] = Ionization_Energy_individual;
                
                double Bin_Xe          = velocity_TH1F[Applied_Hist]->GetXaxis()->FindBin(Ionization_Energy_individual);
                double DCS_Xe          = velocity_TH1F[Applied_Hist]->GetBinContent(Bin_Xe)*1e-15*TMath::Power(Scaling[Index],2);
                DCS_Xe_array[KKK]      = DCS_Xe;
                
                double Bin_Ge          = velocity_TH1F_2[Applied_Hist]->GetXaxis()->FindBin(Ionization_Energy_individual);
                double DCS_Ge          = velocity_TH1F_2[Applied_Hist]->GetBinContent(Bin_Ge)*1e-15*TMath::Power(Scaling[Index],2);
                if(Ionization_Energy_individual>80.)DCS_Ge_array[KKK]      = DCS_Ge;
                else{DCS_Ge_array[KKK]      = 0;}
                
                //double DCS_HH          = DCS_H_c1_0P05GeV_300kms(Ionization_Energy_individual);
                //DCS_HH_array[KKK]      = DCS_HH;
            }

            cout << "Energy_DM(WIMP_mx_Array[kkk],velocitykm[Applied_Hist]*1e3/3e8)*1e3: " << Energy_DM(WIMP_mx_Array[kkk],velocitykm[Applied_Hist]*1e3/3e8)*1e3 << endl;
            TCanvas * c1 = new TCanvas("c", "c", 0,0,1000,1000);
            gStyle->SetOptStat(0);
            gStyle->SetTitleSize(0.04,"XY");
            gStyle->SetTitleFont(62,"XY");
            ...
            TGraph *TGaph_DCS_Xe = new TGraph(Bin_Number_draw,Ionization_Energy,DCS_Xe_array);//Total Cross Section(TCS), Electron Number(EN)
            TGaph_DCS_Xe->SetMarkerStyle(20);
            TGaph_DCS_Xe->SetMarkerColor(3);
            TGaph_DCS_Xe->SetLineColor(3);
            TGaph_DCS_Xe->SetLineWidth(5);
            TGaph_DCS_Xe->GetXaxis()->SetTitle("eV");
            TGaph_DCS_Xe->GetYaxis()->SetTitle("DCS");
            TGaph_DCS_Xe->GetXaxis()->SetRangeUser(12.,Energy_DM(WIMP_mx_Array[kkk],velocitykm[Applied_Hist]*1e3/3e8)*1e3);
            TGaph_DCS_Xe->GetYaxis()->SetRangeUser(1e-45,1e-30);
            TGaph_DCS_Xe->Draw("apl");

            cout << "Energy_DM(WIMP_mx_Array[kkk],velocitykm[Applied_Hist]*1e3/3e8)*1e3: " << Energy_DM(WIMP_mx_Array[kkk],velocitykm[Applied_Hist]*1e3/3e8)*1e3 << endl;
            TGraph *TGaph_DCS_Ge = new TGraph(Bin_Number_draw,Ionization_Energy,DCS_Ge_array);//Total Cross Section(TCS), Electron Number(EN)
            TGaph_DCS_Ge->SetMarkerStyle(20);
            TGaph_DCS_Ge->SetMarkerColor(4);
            TGaph_DCS_Ge->SetLineColor(4);
            TGaph_DCS_Ge->SetLineWidth(5);
            TGaph_DCS_Ge->Draw("plsame");

            TGraph *TGaph_DCS_HH = new TGraph(Bin_Number_draw,Ionization_Energy,DCS_HH_array);//Total Cross Section(TCS), Electron Number(EN)
            TGaph_DCS_HH->SetMarkerStyle(20);
            TGaph_DCS_HH->SetMarkerColor(5);
            TGaph_DCS_HH->SetLineColor(5);
            TGaph_DCS_HH->SetLineWidth(5);
            TGaph_DCS_HH->Draw("plsame");
            ...
            leg->SetBorderSize(0);
            leg->SetTextFont(22);
            leg->AddEntry(TGaph_DCS_Xe,"Xe","lP");
            leg->AddEntry(TGaph_DCS_Ge,"Ge","lP");
            leg->AddEntry(TGaph_DCS_HH,"HH","lP");
            leg->Draw();

            c1->SetLogy();
            c1->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/Predict_DCS_ER/Graph_DCS/%sGeV/Real_%s_DCS.pdf",Mass_s[kkk].c_str(),velocity_s[Applied_Hist].c_str()));
            */

            //Draw the differential cross sections(Straight line)
            /*
            const int Bin_Number_draw=10000;
            double Ionization_Energy[Bin_Number_draw];
            double DCS_Xe_array[Bin_Number_draw];double DCS_Ge_array[Bin_Number_draw];
            ...
            for(int KKK=0; KKK<Bin_Number_draw; KKK++)
            {
                Ionization_Energy[KKK] = 12.+Bin*(KKK+1.);
            ...
            TGaph_DCS_Xe->GetXaxis()->SetTitle("eV");
            TGaph_DCS_Xe->GetYaxis()->SetTitle("DCS");
            TGaph_DCS_Xe->GetXaxis()->SetRangeUser(0,500);
            TGaph_DCS_Xe->GetYaxis()->SetRangeUser(1e-40,1e-30);
            TGaph_DCS_Xe->Draw("apl");
            ...
            */
            ...
            //Change the unit to cm^2/eV double mx, double sigma_SI, double velocity, double atomic_mass, double T
            ...
            double Length_paper    = 2e5;//2km
            double Max_V          = 784.;
            double Cross_section  = CS_Try(1.,1.);
            double Mx             = 1.;
            ...
            double Total=0;
            for(int Ee_array_Number=0; Ee_array_Number<499; Ee_array_Number++)
            {
                //cout << "Ee_array: "        << Ee_array[Ee_array_Number] << endl;
                //cout << "dsigma_dT_array: " << dsigma_dT_array[Ee_array_Number] << endl;
                Total = Total + (Ee_array[Ee_array_Number+1]-Ee_array[Ee_array_Number])*dsigma_dT_array[Ee_array_Number];
                //cout << "Ee_array[Ee_array_Number+1]-Ee_array[Ee_array_Number]: " << Ee_array[Ee_array_Number+1]-Ee_array[Ee_array_Number] << endl;
                //cout << "Total: " << Total << endl;
            }


            double dT = (480.-12.)/500.;
            double Total_cross_section=0;
            double Energy_Max = 0.5*1e9*(1E-3)*(1E-3);
            
            double Ee_Mukesh[500];double dsigma_dT_Mukesh[500];
            
            for(int KKK=0; KKK<500; KKK++)
            {
                double T        = (12.+dT*(KKK+1.));
                Ee_Mukesh[KKK]  = T;
                cout << "dT: " << dT << endl;
                //cout << "T: " << T << endl;
            ...
            TGraph *TGraph_dsigma_dT_Mukesh = new TGraph(500, Ee_Mukesh, dsigma_dT_Mukesh);
            TGraph_dsigma_dT_Mukesh->SetMarkerStyle(20);
            TGraph_dsigma_dT_Mukesh->SetMarkerColor(3);
            TGraph_dsigma_dT_Mukesh->GetXaxis()->SetRangeUser(0,500.);
            TGraph_dsigma_dT_Mukesh->GetYaxis()->SetRangeUser(1E-41,1E-35);
            TGraph_dsigma_dT_Mukesh->GetXaxis()->SetTitle("Ee(eV)");
            TGraph_dsigma_dT_Mukesh->GetYaxis()->SetTitle("DCS");
            TGraph_dsigma_dT_Mukesh->Draw("apl");

            TGraph *TGraph_dsigma_dT_Paper = new TGraph(500, Ee_array, dsigma_dT_array);
            TGraph_dsigma_dT_Paper->SetMarkerStyle(20);
            TGraph_dsigma_dT_Paper->SetMarkerColor(4);
            TGraph_dsigma_dT_Paper->Draw("plsame");
            ...
            Int_t Minimum_Bin_Xe = velocity_TH1F[Applied_Hist]->GetXaxis()->FindBin(12);//Min_Recoil_Bin_Xe
            Int_t Maximum_Bin_Xe = velocity_TH1F[Applied_Hist]->FindLastBinAbove();//Max_Recoil_Bin
            double Xe_Y1 = TMath::Log10(velocity_TH1F[Applied_Hist]->GetBinContent(Minimum_Bin_Xe)*1e-18*TMath::Power(Scaling[Index],2));
            double Xe_Y2 = TMath::Log10(velocity_TH1F[Applied_Hist]->GetBinContent(Maximum_Bin_Xe)*1e-18*TMath::Power(Scaling[Index],2));
            double Xe_X1 = velocity_TH1F[Applied_Hist]->GetXaxis()->GetBinCenter(Minimum_Bin_Xe);
            double Xe_X2 = velocity_TH1F[Applied_Hist]->GetXaxis()->GetBinCenter(Maximum_Bin_Xe);

            Int_t Maximum_Bin_Ge = velocity_TH1F_2[Applied_Hist]->FindLastBinAbove();//Max_Recoil_Bin
            double Ge_Y1 = Xe_Y1;
            double Ge_Y2 = TMath::Log10(velocity_TH1F_2[Applied_Hist]->GetBinContent(Maximum_Bin_Ge)*1e-18*TMath::Power(Scaling[Index],2));
            double Ge_X1 = Xe_X1;
            double Ge_X2 = velocity_TH1F_2[Applied_Hist]->GetXaxis()->GetBinCenter(Maximum_Bin_Ge);
            //cout << "TMath::Power(10, Linear_Line(12.,A_Slope_HH,B_HH)): " << TMath::Power(10, Linear_Line(12.,A_Slope_HH,B_HH)) << endl;
            ...
            for(int KKK=0; KKK<10000; KKK++)
            {
                ...
                cout << "DCS_Xe: " << DCS_Xe << endl;
                Total_Xe_DCS = Total_Xe_DCS + DCS_Xe*Bin;
                Total_Ge_DCS = Total_Ge_DCS + DCS_Ge*Bin;
                Total_HH_DCS = Total_HH_DCS + DCS_HH*Bin;
            }
            ...
            for(int KKK=0; KKK<10000; KKK++)
            {
                double DCS_Xe     =  Scaling_A*TMath::Power(10, Linear_Line(12.+Bin*(KKK+1.),A_Slope_Xe,B_Xe));
                double DCS_Ge     =  Scaling_A*TMath::Power(10, Linear_Line(12.+Bin*(KKK+1.),A_Slope_Ge,B_Ge));
                CCC = CCC + (12.+Bin*(KKK+1.))*(DCS_Xe*Bin/Total_Xe_DCS);
                Energy_Loss_Ge_DCS = Energy_Loss_Ge_DCS + (12.+Bin*(KKK+1.))*(DCS_Ge*Bin/Total_Ge_DCS);
                Energy_Loss_HH_DCS = Energy_Loss_HH_DCS + (12.+Bin*(KKK+1.))*(Scaling_A*DCS_H_c1_1GeV_300kms(12.+Bin*(KKK+1.))*Bin/Total_HH_DCS);
                
                dEdX_total_part = dEdX_total_part + (12.+Bin*(KKK+1.))*(DCS_Ge*Bin);
            }
            ...
            */
            ...
            TGraph *TGraph_dEdX_Paper = new TGraph(200, v_array, dEdX_from_Paper_array);
            TGraph_dEdX_Paper->SetMarkerStyle(0);
            TGraph_dEdX_Paper->SetMarkerColor(0);
            TGraph_dEdX_Paper->GetXaxis()->SetRangeUser(0,784);
            TGraph_dEdX_Paper->GetYaxis()->SetRangeUser(1E-14,1E+14);
            TGraph_dEdX_Paper->GetXaxis()->SetTitle("v[km/s]");
            TGraph_dEdX_Paper->GetYaxis()->SetTitle("dEdX");
            TGraph_dEdX_Paper->Draw("apl");
            ...
            TGraph *TGraph_dEdX_Paper_inverse = new TGraph(200, v_array, dEdX_from_Paper_inverse_array);
            TGraph_dEdX_Paper_inverse->SetMarkerStyle(20);
            TGraph_dEdX_Paper_inverse->SetMarkerColor(5);
            TGraph_dEdX_Paper_inverse->Draw("plsame");
            ...
            leg->SetTextFont(22);
            leg->AddEntry(TGraph_dEdX_Paper,"Paper","lP");
            leg->AddEntry(TGraph_dEdX_Paper_inverse,"1/Paper","lP");
            leg->AddEntry(TGraph_dEdX_Mukesh,"Mukesh","lP");
            leg->AddEntry(TGraph_dEdX_Mukesh_inverse,"1/Mukesh","lP");
            leg->AddEntry(TGraph_dEdX_Ratio,"Ratio(Mukesh/Paper)","lP");
            leg->Draw();
            ...
            */
            ...
            TCS_over_EN->GetYaxis()->SetRangeUser(1E-36,1E-25);
        ...
        */
        ...
        for(int VN=0; VN<velocity_N; VN++)
        {
            ...
            cout << "Energy_Loss_Xe_array[Applied_Hist]/velocity^2 " << Energy_Loss_Xe_array[VN]/(velocitykm[VN]*velocitykm[VN]) << endl;
            cout << "Energy_Loss_Ge_array[Applied_Hist]/velocity^2 " << Energy_Loss_Ge_array[VN]/(velocitykm[VN]*velocitykm[VN]) << endl;
            Energy_Loss_divide_V_square_Xe[VN]=Energy_Loss_Xe_array[VN]/(velocitykm[VN]);
            Energy_Loss_divide_V_square_Ge[VN]=Energy_Loss_Ge_array[VN]/(velocitykm[VN]);
        }
        TCanvas * c1 = new TCanvas("c", "c", 0,0,1000,1000);
        gStyle->SetOptStat(0);
        gStyle->SetTitleSize(0.03,"XY");
        ...
        TG_Energy_Loss_divide_V_square_Xe->GetXaxis()->SetTitle("V(km/s)");
        TG_Energy_Loss_divide_V_square_Xe->GetYaxis()->SetTitle("<E>/V");
        TG_Energy_Loss_divide_V_square_Xe->Draw("apl");
        ...
        c1->SetLogy();
        c1->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/Predict_DCS_ER/Energy_Loss_divide_V_%sGeV.pdf",Mass_s[kkk].c_str()));
         */
        /*
        ...
        TG_Total_Cross_Section_Xe_Times_V_square->SetMarkerColor(2);
        TG_Total_Cross_Section_Xe_Times_V_square->SetMarkerColor(2);
        TG_Total_Cross_Section_Xe_Times_V_square->GetYaxis()->SetRangeUser(1e-40,1e-22);
        TG_Total_Cross_Section_Xe_Times_V_square->GetXaxis()->SetTitle("V(km/s)");
        TG_Total_Cross_Section_Xe_Times_V_square->GetYaxis()->SetTitle("#sigma_{SI}/ V(km/s)");
        TG_Total_Cross_Section_Xe_Times_V_square->Draw("apl");
        ...
        c1->SetLogy();
        c1->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/Predict_DCS_ER/Total_Cross_Section_divide_V_%sGeV.pdf",Mass_s[kkk].c_str()));
         */
        /*
...
for(int Mass_Index=4; Mass_Index<5; Mass_Index++)
{
    ...
    for(int KKK=0; KKK<Bin_Number; KKK++)
    {
        double v    = (v_L+v_R)*0.5;
        double v_c  = (v_L+v_R)*0.5*(1e3/3e8);
        double Total_Cross_v = Total_Cross_Section_Si*v/(300.);
        double energy_loss_v = Energy_Loss_Si*v/(300.);
        double dEdX_total_part = Total_Cross_v*energy_loss_v;
        cout << "dEdX_total_part: " << dEdX_total_part << endl;
        double dE_dX_without_velocity = ND*dEdX_total_part*2e5;
    ...
    cout << "Length_Total: " << Length_Total << endl;
    //cout << "Cross_Section: " << Length_Total/(Length_paper) << endl;
    cout << "cross_Section: " << CS_Try((Length_Total/(Length_paper)),1.) << endl;
}
cout << "=====================================" << endl;
*/
