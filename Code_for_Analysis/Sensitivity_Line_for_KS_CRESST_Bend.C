...
void Sensitivity_Line_for_KS_CRESST_Bend()//CDEX:Threshold=160eV, TEXONO:Threshold=200eV
{
...
for(int kkk=9;kkk<10;kkk++)//Straight or bending
    {
        ...
        cout << "kkk: " << kkk << endl;
        double Mass=0;
    for(int FILE=31; FILE<40; FILE++){//Open1
        cout << "FILE: " << FILE << endl;
    ...
    TFile *ROOT_FILE = TFile::Open(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/%i_STS_Bent%s.root",Mass_Point[Mass_INT].c_str(),FILE,Bent_or_Not_Type[Bent_Type].c_str()));
        TTree *T1_TREE = (TTree*)ROOT_FILE->Get("t1");
            
    TFile *ROOT_FILE1 = TFile::Open(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/%i_STS_Bent%s.root",Mass_Point[Mass_INT].c_str(),FILE,Bent_or_Not_Type[Bent_Type].c_str()));
        TTree *T1_TREE1 = (TTree*)ROOT_FILE1->Get("t1");

        Double_t mx,sigma_si;
        T1_TREE1->SetBranchAddress("mx",&mx);T1_TREE1->SetBranchAddress("sigma_si",&sigma_si);
        T1_TREE1->GetEntry(0);
        ...
        c3->SetLogx();
                cout << "=================================================================================" << endl;
            char fout_name1[300];
            sprintf(fout_name1,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/Recoil_Spectrum/%s_%i_STS_Bent%s.root",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),FILE,Bent_or_Not_Type[Bent_Type].c_str()));
        ...
        if(Take_Plot==1){
        c3->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/Recoil_Spectrum/%s_%i_STS_Bent%s.pdf",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),FILE,Bent_or_Not_Type[Bent_Type].c_str()));
            cout << "=================================================================================" << endl;
        ...
        Threshold_Plot_4->SetLineColor(2);
        Threshold_Plot_4->SetMarkerColor(2);
        Threshold_Plot_4->SetName("Threshold_Plot");
        Threshold_Plot_4->GetXaxis()->SetLimits(1e-42,1e-25);
        Threshold_Plot_4->GetYaxis()->SetRangeUser(1e-42,1e-28);
        Threshold_Plot_4->SetTitle("");
        Threshold_Plot_4->GetXaxis()->SetTitle("Sigma_SI");
        Threshold_Plot_4->GetYaxis()->SetTitle("(Sigma_SI) #times (Event_Fraction)");

        TF1 *Linear_Line = new TF1("Linear_Line","x",1e-42,1e-27);
        Linear_Line->SetLineColor(4);
        
        Threshold_Plot_4->Draw("AL");
        ...
        if(Type_of_Model_INT==0)leg->AddEntry(Threshold_Plot_4,Form("Bin: %i",NU_Bins[kkk]+1),"lP");
        if(Type_of_Model_INT==1)leg->AddEntry(Threshold_Plot_4,Form("Bin: %i",MD_Bins[kkk]+1),"lP");
        if(Type_of_Model_INT==2)leg->AddEntry(Threshold_Plot_4,Form("Bin: %i",BR_Bins[kkk]+1),"lP");
        ...
        {
            char fout_name[300];
            sprintf(fout_name,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/%s_STS%s.root",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),Bent_or_Not_Type[Bent_Type].c_str()));
            TFile *fout=new TFile(fout_name,"recreate");
            Threshold_Plot_4->Write();
            c4->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/2_CRESST_30MWE_MAT/%sGeV/%s_STS%s.pdf",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),Bent_or_Not_Type[Bent_Type].c_str()));
        }
    }
}
