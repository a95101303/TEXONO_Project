...
void Sensitivity_Line_for_CDEX_file5()//CDEX:Threshold=160eV, TEXONO:Threshold=200eV
{
    ...
    int Bent_Type=0; string Bent_or_Not_Type[2]={"_Comparison",""};string Bent_or_Not_Type_Final[2]={"","_Bent"};
    int Air_and_Earth_Type=0; string Air_and_Earth[2]={"","_Earth"};
...
for(int kkk=0;kkk<1;kkk++)
    {
        ...
        double Mass=0;
    for(int FILE=31; FILE<35; FILE++){//Open1
        ...
        TFile *ROOT_FILE = TFile::Open(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/%i_STS_Bent%s.root",Mass_Point[Mass_INT].c_str(),FILE,Bent_or_Not_Type[Bent_Type].c_str()));
        ...
        for(int i=0;i<reso_T;i++)
        {
            ...
            if(T_QF_Original_Bef_Array[i]>0.16)//>Threshold
            {RecoilX_Event_Original_M1  = RecoilX_Event_Original_M1 + Factor1_Original_Bef_Array[i];}
            if(T_QF_Original_Aft_Array[i]>0.16)//>Threshold
        ...
        c3->SetLogx();
                cout << "=================================================================================" << endl;
            char fout_name1[300];
            sprintf(fout_name1,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/Recoil_Spectrum/%s_%i_STS_Bent%s%s.root",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),FILE,Air_and_Earth[Air_and_Earth_Type].c_str(),Bent_or_Not_Type[Bent_Type].c_str()));
        ...
        if(Take_Plot==1){
        c3->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/Recoil_Spectrum/%s_%i_STS_Bent%s%s.pdf",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),FILE,Air_and_Earth[Air_and_Earth_Type].c_str(),Bent_or_Not_Type[Bent_Type].c_str()));
            cout << "=================================================================================" << endl;
        }
        ...
        Threshold_Plot_4->SetLineColor(4);
        Threshold_Plot_4->SetMarkerColor(4);
        ...
        {
            char fout_name[300];
            sprintf(fout_name,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/%s_STS%s%s.root",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),Air_and_Earth[Air_and_Earth_Type].c_str(),Bent_or_Not_Type_Final[Bent_Type].c_str()));
            TFile *fout=new TFile(fout_name,"recreate");
            Threshold_Plot_4->Write();
            c4->Print(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/%s_STS%s%s.pdf",Mass_Point[Mass_INT].c_str(),Type_of_Model[Type_of_Model_INT].c_str(),Air_and_Earth[Air_and_Earth_Type].c_str(),Bent_or_Not_Type_Final[Bent_Type].c_str()));
        }
    }
}
