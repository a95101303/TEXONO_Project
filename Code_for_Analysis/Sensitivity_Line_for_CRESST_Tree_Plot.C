...
void Sensitivity_Line_for_CRESST_Tree_Plot()//
{
...
for(int Bent_or_Not=0; Bent_or_Not<2; Bent_or_Not++)
    ...
    T1_TREE->SetBranchAddress("Energy_Loss_Percentage_lf",&Energy_Loss_Percentage_lf);
                ...
                TH1F   *Y_Earth_Collision_Time = new TH1F("Y_Earth_Collision_Time","Y_Earth_Collision_Time",500,0,500);
                TH1F   *N_Earth_Collision_Time = new TH1F("N_Earth_Collision_Time","N_Earth_Collision_Time",500,0,500);
                ...
                TH1F   *Ratio_of_Air_Origin_After = new TH1F("Ratio_of_Air_Origin_After","Ratio_of_Air_Origin_After",1e2,0,10);
                ...
                for(int Entry=0; Entry<T1_TREE->GetEntries(); Entry++)
                {
                    ...
                    if(Arrival_air==0)
                    {
                        N_Air_Collision_Time->Fill(Collision_Time_Air);
                    }
                    ...
                    Ratio_of_Air_Origin_After->Fill(Path_Length_Air/Oringal_Length_Air);
    ...
    sprintf(fout_name,Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/4_CRESST_Flux/%sGeV/Hist_%i_STS_Bent%s.root",Mass_Point[Mass_INT].c_str(),FILE,Type[Bent_or_Not].c_str()));
                ...
                Ratio_of_Air_Origin_After->Write();
                Y_Air_MFP->Write();
                cout << "Yes  " << endl;
                fout->Close();
            }//fin.is_open()
