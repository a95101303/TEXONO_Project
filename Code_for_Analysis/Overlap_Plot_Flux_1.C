...
void Overlap_Plot_Flux()
{
    ...
    for(int Exp=1; Exp<2; Exp++)
    {
        for(int Mass_INT=10; Mass_INT<11; Mass_INT++)
        {
            ...
            Bef_Flux->GetYaxis()->SetRangeUser(0,1);
            Bef_Flux->Scale(1./5000.);
            ...
            Aft_Flux->GetYaxis()->SetLimits(0,1);
            Aft_Flux->Scale(1./5000.);
            ...
            leg->AddEntry(Bef_Flux,"Before_attenuation","l");
            leg->AddEntry(Aft_Flux,"After_attenuation","l");

            Bef_Flux->Draw("HIST");
            Aft_Flux->Draw("HISTsame");
                MD_Line_STFlux->Draw("Lsame");
            leg->Draw();
            c3->Print(Form("/Users/yehchihhsiang/Desktop/Analysis/CDEX_Analysis_method/Codes/%s/%sGeV/Flux/%i.pdf",Exp_Flux[Exp].c_str(),Mass_Point[Mass_INT].c_str(),FILE));
            }
        }
    }
}
