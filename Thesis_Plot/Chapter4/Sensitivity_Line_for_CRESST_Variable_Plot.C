...
void Sensitivity_Line_for_CRESST_Variable_Plot()
{
    ...
    int FILE[2]={49,50};//Earth and Air
    string Plot[2]={"Ratio_of_Earth_Origin_After","Ratio_of_Air_Origin_After"};
    ...
    for(int FILE_Index=0; FILE_Index<2; FILE_Index++)
    {
        for(int Exp=1; Exp<2; Exp++)
        {
            for(int Mass_INT=1; Mass_INT<2; Mass_INT++)
            {
                ...
                TFile *fin2 = TFile::Open(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/%s/%sGeV/Hist_%i_STS_Bent.root",Exp_Flux[Exp].c_str(),Mass_Point[Mass_INT].c_str(),FILE[FILE_Index]));
                    Bent_Line=(TH1F*)fin2->Get(Plot[FILE_Index].c_str());
                    Bent_Line->SetLineColor(2);
                    Bent_Line->SetLineStyle(1);
                Bent_Line->SetTitle("");
                Bent_Line->GetYaxis()->SetRangeUser(0,1);
                Bent_Line->GetXaxis()->SetRangeUser(0,4);
                    Bent_Line->Scale(1/(Bent_Line->GetEntries()));

                TFile *fin3 = TFile::Open(Form("/Users/yehchihhsiang/Desktop/GITHUB_TEXONO/%s/%sGeV/Hist_%i_STS_Bent_Comparison.root",Exp_Flux[Exp].c_str(),Mass_Point[Mass_INT].c_str(),FILE[FILE_Index]));
                    Straight_Line=(TH1F*)fin3->Get(Plot[FILE_Index].c_str());
            ...
            //Bent_Line->GetYaxis()->SetRangeUser(0,200);
                ...
                c3->Print(Form("%i_%s.pdf",FILE[FILE_Index],Plot[FILE_Index].c_str()));
            }
        }
    }
